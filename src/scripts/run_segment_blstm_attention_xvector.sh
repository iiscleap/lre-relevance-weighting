...
data=data/train
nnet_dir=exp/xvector_nnet_attn_1a/
egs_dir=exp/xvector_nnet_attn_1a/egs

. ./utils/parse_options.sh

num_pdfs=$(awk '{print $2}' $data/utt2spk | sort | uniq -c | wc -l)
#echo '9' > ${egs_dir}/info/left_context
#echo '300' > ${egs_dir}/info/right_context
...
if [[ ${stage} -le 5 ]]; then
    ...
    stats-layer name=segment-stats config=mean+stddev(-49:1:49:49)
    relu-batchnorm-layer name=tdnn5_1 dim=512

    fast-lstmp-batchnorm-layer name=blstm1-forward input=tdnn5_1 delay=-1 $lstm_opts
    fast-lstmp-batchnorm-layer name=blstm1-backward input=tdnn5_1 delay=1 $lstm_opts
    #fast-lstmp-batchnorm-layer name=blstm2-forward input=Append(blstm1-forward, blstm1-backward) delay=-2 $lstm_opts
    #fast-lstmp-batchnorm-layer name=blstm2-backward input=Append(blstm1-forward, blstm1-backward) delay=2 $lstm_opts

    self-layer name=attention1 config=mean(0:1:1:${max_chunk_size}) input=Append(blstm1-forward, blstm1-backward) num-heads=1 affine-dim=128
    relu-batchnorm-layer name=tdnn6 dim=512 input=attention1
